<div class="max-w-3xl mx-auto mt-12 p-8 bg-white/70 shadow-2xl rounded-3xl border-t-4 border-orange-500 ring-1 ring-orange-100 backdrop-blur-xl transition-all duration-500">
    ...
    <div class="flex items-center justify-between mb-10">
        ...
        <div class="flex-1 text-center relative">
            ...
            @if ($step < 3)
                <div class="absolute top-5 sm-left-[-4rem] md:left-[-6rem] lg:md:left-[-6rem] left-[-3rem] w-4/5 h-px bg-orange-200">
        </div>
        @endif
    </div>
    @endforeach
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <label class="block">
        ...
        <div
            class="mt-2 w-full rounded-xl border-orange-200 shadow px-4 py-3 bg-white/40 backdrop-blur-lg text-gray-800 cursor-default select-text">
            {{ $code }}
        </div>
    </label>
...
<div class="bg-orange-50 border border-orange-200 rounded-2xl p-6 shadow-inner text-orange-800 space-y-4 backdrop-blur-lg">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        ...
        <div class="md:col-span-2">
            ...
            <div class="mt-2">
                @if (is_string($image))
                <img src="{{ $image }}" onerror="this.onerror=null; this.src='{{ Storage::url($image) }}';" alt="الصورة" class="h-24 w-24 rounded-lg shadow object-cover">
                @else
